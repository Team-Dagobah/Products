...
app.get('/products', (req, res) => {
  ...
  let count = req.query.count || 5
  if(page === 1){
    return db.queryAsync(`EXPLAIN ANALYZE Select * from products order by id asc limit ${count}`)
    ...
    return db.queryAsync(`EXPLAIN ANALYZE Select * from products where id > ${(page - 1)*count} order by id asc limit ${count}`)
    .then(result => result[0])
    .then((result) => {
      res.status(200).send(result.rows)
    })
